...
//추상클래스에 대해서 알아보기 위해 우리가 작성했던 Predator 인터페이스를 다음과 같이 추상클래스로 변경해 보자. 
abstract class Predator extends Animal{
	...
	static int speed() {
		return LEG_COUNT * 30;
		// 추상 클래스를 만들기 위해서는  class 앞에 abstract라고 표기해야 한다. 
		...
		// 그리고 Animal클래스의 기능을 유지하기 위해 Animal클래스를 상속했다.
		// 그리고 인터페이스의 디폴트 메서드는 더이상 사용할 수 없으므로 default 키워드를 삭제하여
		// 일반 메서드로 변경했다. 그리고  LEG_COUNT상수도 인터페이스에서는 자동으로 static 으로 인식하지만 추상 클래스는
		// 명시적으로 static이라고 적어 주어야 한다. 
		// 추상 클래스는 일반 클래스와는 달리 단독으로 객체를 생성할 수 없다. 반드시 추상 클래스를 상속한 실제 클래스를 통해 서만 
		// 객체를 생성할 수 잇따.
		// Predator 인터페이스를 이와 같이 추상클래스로 변경하면 BarkablePredator 인터페이스는 사용 불가능 하므로 삭제해야 하고
		// Tiger,Lion 클래스도 다음과 같이 Predator 추상클래스를 상속하도록 변경 해야 한다. 
	}
	...
	// 추상 클래스에는 abstract 메소드 외에 실제 메소드도 사용할수 있다. 
